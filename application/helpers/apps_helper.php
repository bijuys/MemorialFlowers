<?php
    ...
    {
    	return base_url('templates/memorial/'.$path);
    }
        	...
        	foreach($query->result() as $row)
        	{
				...
				if($page==$row->menuitem){
            		...
            		if($type=='li')
            		{
                		...
                		if(ucwords($page)==$row->menuitem)
                    			$menu .= ' class="current"';
							if($row->menuitem_id==156 || $row->menuitem_id==157){
								$menu .= '><a href="'.base_url().''.$row->menulink.'"><h4 style="color:#41C1C0;'.$sty_tag.'">'.lang(trim($row->menuitem)).'</h4></a>'."\n";
							}else{
                		...
                		if($submenu==1 && $row->subitems>0)
                		{
			              ...
			              foreach($query->result() as $line)
			              {
								...
								if($page==$line->menuitem){
			                        ...
			                        $menu .= "\n".'<li><a href="'.base_url().''.$line->menulink.'" style="'.$sty_tag.'">'.lang($line->menuitem)."</a></li>\n";
			              }
...
function new_calendar($month,$year){
	...
	if(strtotime($year.'-'.$month.'-'.'01') > strtotime(date('Y-m-01',time())))
		$calendar .= ' href="#cal_'.$prevmonth->format('mY').'" ><i class="glyphicon glyphicon-chevron-left"></i></a>';
	else
		$calendar .= ' href="#" ><i class="glyphicon glyphicon-chevron-left" style="color: #DDD;"></i></a>';

	$calendar .= '</div>';
	$calendar .= '<div class="pull-right"><a  class="next" class="btn btn-xs btn-default"';

	//$calendar .= ' href="#cal_'.$nextmonth->format('mY').'" ><i class="glyphicon glyphicon-chevron-right"></i></a> ';
	

	if(strtotime($year.'-'.$month.'-'.'01') < strtotime(date('Y-m-t',strtotime('+11 months'))))
		$calendar .= ' href="#cal_'.$nextmonth->format('mY').'" ><i class="glyphicon glyphicon-chevron-right"></i></a> ';
	else
		$calendar .= ' href="#" ><i class="glyphicon glyphicon-chevron-right" style="color: #DDD;"></i></a> ';
