...
class Affiliates extends CI_Controller{
	...
	{
		$string = $_POST['search'];
		$total= $this->Affiliate_model->count_affiliates();
		$per_pg=20;
		$offset=$this->uri->segment(4);
		
		$this->load->library('pagination');
		$config['base_url'] = $this->config->item('base_url').'/siteadmin/afiliates/browse';
		...
		$data['this_class'] = get_class($this);
		$data['affiliates'] = $this->Affiliate_model->search_affiliates($string);
		$this->load->view('admin/affiliates',$data);
	}
	...
	function create()
	{
		...
		{
			$this->Affiliate_model->create($this->input->post());
			redirect('siteadmin/affiliates');
			exit;
		}
	}
	...
	function edit($id)
	{
		$this->form_validation->set_rules('user_name', 'User Name','required|min_length[5]|callback_unique_check[user_name,'.$id.']');
		$this->form_validation->set_rules('user_password', 'Password','required');
		...
		if ($this->form_validation->run() == FALSE)
		{
			if( ! $_POST)
			{
				$data['affiliate'] = $this->Affiliate_model->get_affiliate($id);
			}
			$this->load->model('Country_model');
			$this->load->model('Province_model');
			$data['this_class'] = get_class($this);
		...
		{
			$this->Affiliate_model->update($this->input->post());
			redirect('siteadmin/affiliates');
			exit;
		}
	}
