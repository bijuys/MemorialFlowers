...
class Keywords extends CI_Controller{
	...
	function index()
	{
		...
		$data['landing_pa'] = $this->Keyword_model->allkeyword();
	...
	function create()
	{
		...
		$this->form_validation->set_rules('folder_name', 'folder_name','required');
		$this->form_validation->set_rules('folder_link', 'folder_link','required');
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->load->view('admin/keyword-form',$data);
		...
		else
		{
			...
			/*if(!$image = $this->_doUpload($_POST['landing_city']))
				$image='';*/
			$this->Keyword_model->create($this->input->post());
			//$this->Productgroup_model->create($this->input->post());
			
			redirect('siteadmin/keyword');
			exit;
