      ...
      <div class="row-fluid">
        <div class="span12" style="height:600px;">
			...
			$result = mysqli_query($con,"SELECT * FROM product_prices WHERE product_id='".$product->product_id."' ORDER BY price_value ASC");
					$nu = 1;
					?>
					<select name="price-<?php echo $product->product_id; ?>" id="price-<?php echo $product->product_id; ?>">
					<?php
					while($row = mysqli_fetch_array($result)){ ?>
						
						<option value="<?php echo $row['price_id']; ?>"><?php echo $row['price_name']; ?> - C$<?php echo $row['price_value']; ?></option>
        ...
        <div class="span12">
            ...
            <a href="#" id="<?php echo $product->product_id;?>" class="selectbox btn btn-inverse btn-mini"><?php echo lang('order');?></a>
              <!--<a href="#" id="<?php //echo $product->product_id;?>" class="selectbox btn btn-inverse btn-mini" onclick="location.reload();location.href='http://new.memorialflowers.ca/mymemorial/orders/create/0'"><?php //echo lang('order');?></a>-->
