...
$(function(){
  ...
  function init()
  {
    $('.selectbox').unbind('click').click(function(event){
      event.preventDefault(event);
        ...
        $.post('/mymemorial/orders/addcart',{'product_id':$(this).attr('id'),'price_id':$("#price-"+$(this).attr('id')+" option:selected").val(),'csrf1800':csrf}, function(data) {
          
           $.get('/mymemorial/orders/mycart',{'keyword':''},function(data){
  ...
  function cart_init()
  {
    $('.remove').bind('click',function(){
      ...
      $.get('/mymemorial/orders/mycartrem',{'id':rid},function(data) {
                $("#mycart").html(data.sidecart);
                $("#cart-details").html(data.cartview);
                $("#cart-total").html(data.cartotal);
                cart_init();  
  ...
  $('.next').click(function(event){
     ...
     else if(nid=='step-two')
     {
        ...
        $.post('/mymemorial/orders/deliveryaddress',{'csrf1800':csrf,'firstname':$("#firstname").val(),
                                                      ...
                                                      "locationtype":$("#locationtype").val()}, function(data){
              ...
              if(data.validation=='ok')
              {
                ...
                $.post('/mymemorial/orders/itemdelivery',{'csrf1800':csrf,'province':$("#province").val(),
                    ...
                    while(i<count)
                    {
                        ...
                        var queryDate = '{$nextday}';
                        
                        var parsedDate = $.datepicker.parseDate('yy-mm-dd', queryDate);
                        
                        $('#datepicker'+i).datepicker('setDate', parsedDate);
                        
                        var day1 = $("#datepicker"+i).datepicker('getDate').getDate();                 
                        var month1 = $("#datepicker"+i).datepicker('getDate').getMonth() + 1;             
                        var year1 = $("#datepicker"+i).datepicker('getDate').getFullYear();
                        var fullDate = month1 + "-" + day1 + "-" + year1;
                        var altDate = year1 + "-" + month1 + "-" + day1;
                      
                        $("#delivery_date"+i).val(altDate);
              ...
              <div class="sectionbody" id="sectionbody-1">
                ...
                <div id="psearch" class="clearfix">
                  <form action="" method="post" name="psearch" >
                    <label for="keyword"><?php lang('Search Products');?></label>
