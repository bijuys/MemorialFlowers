...
<?php include_once("header.php"); ?>
    <div id="main" class="clearfix">
      ...
      <div id="content" class="clearfix">      
        ...
        <p><label for="affiliate_id">Select an Affiliate</label>
            <select name="affiliate_id" id="affiliate_id">
                  ...
                  <option value="<?php echo $row->affiliate_id;?>" <?php if($order->affiliate_id==$row->affiliate_id) echo 'selected="selected"'; ?> ><?php echo $row->firstname . ' '. $row->lastname;?> (<?php echo '#'.$row->affiliate_id;?>)</option>
                  <?php endforeach; ?>
            </select>
        </p>
        <p><label for="product_id">Select Product/Prices</label>
            <select name="price_id[]" id="price_id" multiple="multiple" size="15" style="width:500px; font-size: 90%;" >
                  ...
                  <option value="<?php echo $row->price_id;?>" <?php if($order->price_id==$row->price_id) echo 'selected="selected"'; ?>><?php echo $row->product; ?> ( <?php echo $row->title.'- $'.number_format($row->price,2);?>)</option>
                  <?php endforeach; ?>
            </select>
        </p>
        <p>
      ...
      <select id="locationtype" name="locationtype">
            ...
            <option value="Funeral Home" <?php if($order->locationtype=='Funeral Home') echo 'selected="selected"'; ?>>Funeral Home</option>
            <option value="Chapel" <?php if($order->locationtype=='Chapel') echo 'selected="selected"'; ?>>Chapel</option>                          
      </select></p>
        <p><label for="firstname">Firstname</label>
            <input type="text" name="firstname" size="35"  value="<?php echo $order->firstname;?>" id="firstname" />
        </p>
        <p><label for="lastname">Lastname</label>
            <input type="text" name="lastname" size="35"  value="<?php echo $order->lastname;?>" id="lastname" />
        </p>
        <p><label for="address">Address</label>
            <input type="text" name="address" size="35"  value="<?php echo $order->address;?>" id="address" />
        </p>
        <p><label for="postalcode">Postalcode</label>
            <input type="text" name="postalcode" size="35"  value="<?php echo $order->postalcode;?>" id="postalcode" />
        </p>
        <p><label for="city">City</label>
            <input type="text" name="city" size="35"  value="<?php echo $order->city;?>" id="city" />
        </p>
        <p><label for="province_id">Province</label>
            <select name="province_id" id="province_id">
                  <?php foreach($provinces as $row) : ?>
                  <option value="<?php echo $row->province_id;?>" <?php if($row->province_id==$order->province_id) echo 'selected="selected"'; ?>><?php echo $row->province;?></option>                  
                  <?php endforeach; ?>
            </select>
        </p>
        <p><label for="country_id">Country</label>
            <select name="country_id" id="country_id">
                  ...
                  <option value="<?php echo $row->country_id;?>"  <?php if($row->country_id==$order->country_id) echo 'selected="selected"'; ?>><?php echo $row->country;?></option>                  
                  <?php endforeach; ?>
            </select>
        </p>
        <p><label for="phone">Phone</label>
            <input type="text" name="phone" size="35"  value="<?php echo $order->phone;?>" id="phone" />
        </p>
        </fieldset>
        <fieldset>
              ...
              <select name="deliverytime[]" id="deliverytime">
                  <?php for($t=6;$t<=23;$t++) :
                          for($m=0;$m<=45;$m=$m+30) :
                              ...
                              <option value="<?php echo date('h:ia',$time);?>"  <?php if(date('h:ia',$time)==$order->gtime) echo 'selected="selected"'; ?>><?php echo date('h:ia',$time);?></option>
                  <?php   endfor;
                        endfor; ?>
              </select>
      </p>
      <p><label for="businessname">Order PO</label>
            <input type="text" name="businessname[]" id="businessname" value="<?php echo $order->businessname;?>" />
      </p>
      <p><label for="orderedby">Ordered By</label>
            <input type="text" name="orderedby[]" id="orderedby"  value="<?php echo $order->orderedby;?>"/>
      </p>
        <p><label for="message">Card Message</label>
            <textarea name="message[]" cols="60" rows="5" ><?php echo $order->message;?></textarea>
        </p>
        </p>
            <p><label for="ribbontext">Ribbon Text</label>
            <textarea name="ribbontext[]" cols="60" rows="5"><?php echo $order->ribbontext;?></textarea>
        </p>
        </p>
            <p><label for="specialnotes">Special Notes</label>
            <textarea name="specialnotes[]" cols="60" rows="5"><?php echo $order->specialnotes;?></textarea>
        </p>
        </fieldset>
        <p><label>&nbsp;</label>
