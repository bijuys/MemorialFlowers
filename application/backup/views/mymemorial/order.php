...
$maxdate = date('Y, m-1, d',strtotime('+6 months'));
...
$(function(){
  ...
  $('.next').click(function(event){
     ...
     else if(nid=='step-three')
     {
		...
		var items_count = $("#datepicker_count").val();
		items_count++;		 	
     	alert_text = "";
		...
		for ( var i = 1; i < items_count; i++ ) 
		{
			var item_alert = "";		 			
			 ...
			 if( $.trim( $('#datepicker'+i).val() ) == "" || $.trim( $('#datepicker'+i).val() ) == "0000-00-00")
             {
				 item_alert += " Please select a 'Delivery Date' for Item #"+ i +"\\n";
				
			 }
			 
			 if( $.trim( $('#deliverytime'+i).val() ) == "" )
             {
				 item_alert += " Please give 'Delivery Time' for Item #"+ i +"\\n";
			
			 }
			 
			 if( $.trim( $('#businessname'+i).val() ) == "" )
             {
				 item_alert += " Please give 'Order PO' for Item #"+ i +"\\n";
				
			 }
			 
			 if( $.trim( $('#orderby'+i).val() ) == "" )
             {
				 item_alert += " Please give 'Order By' for Item #"+ i +"\\n";
				
			 }		
			 
			 var nm = "ribbon_color["+$('#rb'+i).val()+"]";		 
			 var nm_val = $("input:radio[name ='"+nm+"']:checked").val();	 
		
			 if( nm_val  != "No Ribbon" &&  $.trim($('#ribbontext'+i).val()) == "" )
             {				 
				item_alert += " Please give  the 'Ribben Text' for Item #"+ i +"\\n";	
									 
			 }	 
			 
			 if( nm_val  == "No Ribbon" &&  $.trim($('#ribbontext'+i).val()) != "" )
             {				 
				item_alert += " Please select 'Ribbon Color' for Item #"+ i +"\\n";	
          ...
          <div class="contents">
            ...
            <div class="section">
              ...
              <div class="sectionbody" id="sectionbody-1">
                ...
                <br /><br />
                <div id="products-wrap" style="margin-top:32px; float: none; clear: both;">
...
<?php echo lang('Total');?>: <span id="cart-total"></span></span><button class="next btn btn-success" id="step-one"><?php echo lang('Next Step');?></button></div>
            ...
            <?php 
				...
				if( $this->session->userdata('date_error') == 1)
				{
			?>
			
				<script type="text/javascript">
                alert(" Please give 'Delivery date' on 'Step 3: Delivery Details' ");
                </script>
				
			<?php	
					$this->session->unset_userdata('date_error');
				}
			...
			echo form_open(base_url().'mymemorial/orders/submit',array('id'=>'orderform','name'=>'orderform'));?>
    ...
    function getCity(strURL) {      
        ...
        if (req) {
            ...
            req.send(null);
              ...
              <!--   shaji     <option value="<?php echo $r['name'];?>"><?php echo $r['name'];?></option> -->
					 <?php } 
					 }else 
					 {
						$sqlu = mysqli_query($con,"SELECT * FROM users where user_id=".
								$this->session->userdata('affiliate_id')." and user_role='affiliate'"); 
						$ru=mysqli_fetch_assoc($sqlu);
						echo '<option value="'. $ru["user_firstname"].' '.$ru["user_lastname"].'">'.
						   $ru["user_firstname"].' '.$ru["user_lastname"] .'</option>';
				...
				<div id="citydiv">
                   ...
                   <div class="formcol1 form-horizontal">     
                    ...
                    <div class="control-group">
                      <label for="firstname" class="control-label"><?php echo lang('Deceased Firstname');?></label>
                      <div class="controls">
                      <?php // echo '<input type="text" name="firstname" size="30" id="firstname" value="'.$rs['contact_firstname'].'" class="rounded" />'; 
					   ...
					   echo '<input type="text" name="firstname" size="30" id="firstname" value="" class="rounded" />';
					  ?>
                    ...
                    <div class="control-group">
                      <label for="lastname" class="control-label"><?php echo lang('Deceased Lastname');?></label>
                      <div class="controls">
                       <?php  //echo '<input type="text" name="lastname" size="30" id="lastname" value="'.$lname.'"/>';
								echo '<input type="text" name="lastname" size="30" id="lastname" value=""/>';					   ?>
