...
(function($) {
    ...
    // Add a reverse reference to the DOM object
      ...
      // Top and tail the list with 'visible' number of items, top has the last section, and tail has the first
		...
		var $itemClone1 = base.$items.filter(':last').clone().addClass('cloned');
        var $itemClone3 = base.$items.filter(':first').clone().addClass('cloned');
        var $itemClone4 = base.$items.filter(':first').next().clone().addClass('cloned');
        var $itemClone5 = base.$items.filter(':first').next().next().clone().addClass('cloned');
		
		if ($itemClone1.attr("id") != "") {
          base.$items.filter(':first').before($itemClone.attr('id', ($itemClone.attr('id') + "-cloned")));
        } else {
          base.$items.filter(':first').before($itemClone1);
        }

        //$itemClone = base.$items.filter(':first').clone().addClass('cloned');
        if ($itemClone3.attr("id") != '') {
          base.$items.filter(':last' ).after($itemClone3.attr('id', ($itemClone3.attr('id') + "-cloned")));
        } else {
          base.$items.filter(':last' ).after($itemClone5);
          base.$items.filter(':last' ).after($itemClone4);
          base.$items.filter(':last' ).after($itemClone3);
    ...
    // Bind to the forward and back buttons
      ...
      $forward.click(function(e) {
		...
		base.buildAutoPlay = function(){
			...
			base.$startStop = $("").html(base.playing ? '':'');
